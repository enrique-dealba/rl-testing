...
class SOAP(optim.Optimizer):
    ...
    def _print_sizes(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # Momentum buffers
                exp_avg_size += state["exp_avg"].numel()
                exp_avg_bytes += (
                    state["exp_avg"].numel() * state["exp_avg"].element_size()
                )
                exp_avg_sq_size += state["exp_avg_sq"].numel()
                exp_avg_sq_bytes += (
                    state["exp_avg_sq"].numel() * state["exp_avg_sq"].element_size()
                )

                # Preconditioners (including both GG and Q)
                for precond in ["GG", "Q"]:
                    if precond in state:
                        for tensor in state[precond]:
                            if isinstance(tensor, torch.Tensor):
                                precond_size += tensor.numel()
                                precond_bytes += tensor.numel() * tensor.element_size()

        exp_avg_size_mb = exp_avg_bytes / (1024 * 1024)
        exp_avg_sq_size_mb = exp_avg_sq_bytes / (1024 * 1024)
        precond_size_mb = precond_bytes / (1024 * 1024)
        total_size = exp_avg_size + exp_avg_sq_size + precond_size
        total_size_mb = (exp_avg_bytes + exp_avg_sq_bytes + precond_bytes) / (
            1024 * 1024
        )

        print(f"SOAP exp_avg size: {exp_avg_size} elements, {exp_avg_size_mb:.2f} MB")
        print(
            f"SOAP exp_avg_sq size: {exp_avg_sq_size} elements, {exp_avg_sq_size_mb:.2f} MB"
        )
        print(
            f"SOAP Preconditioners size: {precond_size} elements, {precond_size_mb:.2f} MB"
        )
        print(f"SOAP Total size: {total_size} elements, {total_size_mb:.2f} MB")
    ...
    def step(self):
        ...
        # Print sizes after the first complete step
